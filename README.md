...
- **Commenting inside a table, code fence, or `$$` math block anchors the whole block.** Inline markers would corrupt those structures (a table row must stay one line), so the anchor snaps to the block's boundaries with markers on their own lines; `quote:` still records the text you selected. Frontmatter can't be anchored at all — use a whole-file comment.
